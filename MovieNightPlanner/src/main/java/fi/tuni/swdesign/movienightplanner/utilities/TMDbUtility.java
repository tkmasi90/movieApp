...
/**
 * The {@code Constants} class provides a set of constant values,
 * configurations, and utility methods for managing supported languages,
 * streaming providers, and generating URLs for movie data from The Movie
 * Database (TMDb) API. It includes predefined language pairs, streaming
 * provider IDs, and methods to retrieve provider-related data and formatted
 * URLs for popular, top-rated, and filtered movies.
 *
...
public final class TMDbUtility {
    ...
    /**
     * Represents a collection of provider IDs for streaming services supported
     * by the application. Each constant field represents a unique provider
     * identifier.
     */
    ...
    /**
     * Retrieves a list of streaming provider IDs by using reflection to access
     * the static fields in the {@code PROVIDERS} class.
     *
    ...
    /**
     * Encodes the provider IDs into a URL-friendly string, where IDs are
     * concatenated and separated by the '|' character.
     *
     * @return a URL-encoded string representing the provider IDs, or
     * {@code null} if encoding fails
     */
    ...
    /**
     * Encodes a given list of provider IDs into a URL-friendly string, where
     * IDs are concatenated and separated by the '|' character.
     *
    ...
    /**
     * Encodes a given list of genres and languages into a URL-friendly string,
     * where strings are concatenated and separated by the '|' character.
     *
     * @param list the list of genres or languages to encode
     * @return a URL-encoded string representing the genres or languages, or
     * {@code null} if encoding fails
     */
    ...
    /**
     * Generates the URL for retrieving popular movies from TMDb, with the list
     * of providers encoded as a URL parameter.
     *
    ...
    /**
     * Generates the URL for retrieving top-rated movies from TMDb, sorted by
     * vote average and including only providers supported by the app.
     *
    ...
    public String getSearchUrl(String query) {
        try {
            ...
            String url = String.format(MOVIE_SEARCH_URL, encodedQuery);
            return url;
    ...
    /**
     * Generates a formatted URL for discovering movies from TMDb based on
     * specified filtering criteria. The URL incorporates filters for genres,
     * audio languages, streaming providers, and minimum rating.
     *
     ...
     * @param provList A list of streaming provider IDs to include in the
     * filter.
     * @param minRating The minimum rating to include in the filter.
     * @return A formatted URL string with the applied filters, ready for use in
     * TMDb API requests.
     */
    ...
    private final String MOVIE_SEARCH_URL = "https://api.themoviedb.org/3/search/movie?query=%s&include_adult=false&language=en-US&page=1&region=FI";
}
